...
#        self.move_particles(w)
        ...
        load_balance = LoadBalance(self.pc, corner, box_length, comm, self.factor, self.order)
        tree = load_balance.decompisition()

        # update neighbors in load balance partitions and external boundaries
        self.boundary.update_internal_ghost_particles(tree)
        self.boundary.update_external_ghost_particles(tree)

        # locally create the mesh

        # solve gravity 

        # update hydro equations
