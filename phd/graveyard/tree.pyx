...
cdef class BaseTree:
    ...
    cdef int _get_nearest_process_neighbors(self, double[:] center, double h, np.int32_t[:] leaf_proc, int rank, LongArray nbrs):
        cdef int i
        cdef double smin[3]
        cdef double smax[3]
    ...
    cdef void _neighbors(self, Node* node, double smin[3], double smax[3], np.int32_t[:] leaf_proc, int rank, LongArray nbrs):
        ...
        if node.children_start == -1:
            ...
            if rank != leaf_proc[node.array_index]:
                nbrs.append(leaf_procs[node.array_index])
            ...
            for i in range(1 << self.dim):
                self._neighbors(node + node.children_start + i, smin, smax, nbrs)
