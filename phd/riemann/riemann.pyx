...
#
#            # newton rhapson 
#            p_star = get_pstar(_dl, Vnl, _pl, _dr, Vnr, _pr, gamma)
#
#            # calculate the contact wave speed
#            f_l = p_func(_dl, Vnl, _pl, gamma, p_star)
#            f_r = p_func(_dr, Vnr, _pr, gamma, p_star)
#            u_star = 0.5*(Vnl + Vnr + f_r - f_l)
#
#            if(0.0 <= u_star):
#
...
#                            v_sq = vn = 0.
#                            for k in range(dim):
#                                _v[k] = vl[k][i] + (u_start - vnl)*nx[k][i]
#                                vn += _v[k]*nx[k][i]
#                                v_sq += _v[k]*_v[k]
#
#                            fm.data[i]  = _d*vn
...
#
#                    # left shock
#                    s_l = vnl - cl*sqrt((gamma + 1.0)*p_star/(2.0*gamma*pl) + (gamma - 1.0)/(2.0*gamma))
#
#                    if(0.0 <= s_l):
#
#                        # left state
...
#
#                    # right shock
#                    s_r = vnr + cr*sqrt((gamma + 1.0)*p_star/(2.0*gamma*_pr) + (gamma-1.0)/(2.0*gamma))
#
#                    if(0.0 >= s_r):
#
#                        # right data states
...
#                            v_sq = vn = 0.
#                            for k in range(dim):
#                                _v[k] = vr[k][i] + (u_start - vnr)*nx[k][i]
#                                vn += _v[k]*nx[k][i]
#                                v_sq += _v[k]*_v[k]
#
#                            fm.data[i]  = _d*vn
