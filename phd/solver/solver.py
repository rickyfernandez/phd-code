...
#    plt.plot(pc['position-x'][~real], pc['position-y'][~real], 'rx')
#
#    boundary = pc['type'] == ParticleTAGS.Boundary
#    plt.plot(pc['position-x'][boundary], pc['position-y'][boundary], 'ko')
...
#        iteration_count=0, current_time=0, conservation_check=True, comm=None):
#        """Constructor
#
#        Parameters:
#        -----------
...
#                self.boundary.update_ghost_particles()
#                self.mesh.tessellate()
#
#            # compute initial cell information (vol, area, ..)
#            self.mesh.compute_cell_info()
#
#            # I/O
#            if iteration_count % self.pfreq == 0:
#                self.save(iteration_count, current_time, dt)
...
#
#        output_dir = self.path + "\\" + `self.output`.zfill(4)
#
#        if self.rank == 0:
#            os.mkdir(output_dir)
#        self.comm.barrier()
