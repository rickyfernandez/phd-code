...
num_proc = 4
io = phd.Hdf5()
base_name = "explosion_mpi_output/final_output/final_output0000"
for i in range(num_proc):
    ...
    if i == 0:
        explosion = io.read(file_name)
    else:
        explosion.append_container(io.read(file_name))
tag = np.where(explosion["type"] == phd.ParticleTAGS.Interior)[0]
explosion.remove_items(tag)
...
plt.tight_layout()
plt.savefig("explosion_2d_mpi.png")
plt.show()
